...
```mermaid
flowchart LR
    A[Mikrocirkel-Initiering] --> B[Nodstabilisering]
    B --> C[Nodreplikering]
    C --> D[Nätverkad Federation]
    D --> E[Kulturell Adoption]

    subgraph Återkopplingsloopar
        B -->|Kunskapsöverföring| A
        C -->|Mönsterförfining| B
        D -->|Protokollåterkoppling| C
        E -->|Kulturell Anpassningsinsikt| D
    end

    style A fill:#FFDDC1,stroke:#333,stroke-width:2px
    style B fill:#FFD580,stroke:#333,stroke-width:2px
    style C fill:#FFE680,stroke:#333,stroke-width:2px
    style D fill:#D1FFB8,stroke:#333,stroke-width:2px
    style E fill:#B8FFF2,stroke:#333,stroke-width:2px
```
